---
date: 2023-01-01T00:30:05.000Z
layout: post
published: true
title: "[JavaScript] Execution Context"
subtitle: What is a Execution Context?
description: Let's find out the Execution Context in JavaScript
...
> 실행 컨텍스트는 자바스크립트의 핵심 동작 원리를 담고 있는 개념이다. 실행컨텍스트를 이해하면 스코프체인, 호이스팅, 클로저와 태스크큐 등의 처리 방식을 이해할 수 있다.

## 소스코드의 타입

>  자바스크립트에는 4가지 소스코드 타입이 있고 각기 다른 실행 컨텍스트를 생성된다.

####1. 전역 코드

전역에 존재하는 소스코드. 전역에 정의된 함수, 클래스 등의 내부 코드는 포함되지 않는다.

전역 변수를 관리하기 위해 최상위 스코프인 전역 스코프를 생성해야 하고,  var 키워드로 선언된 전역 변수와 함수 선언문으로 정의된 전역 함수를 전역 객체의 프로퍼티와 메서드로 바인딩하기 위해 전역 객체와 연결해야한다. 이를 위해 전역 코드가 평가되면 전역 실행 컨텍스트가 생성된다.

#### 2. 함수 코드

함수 코드는 지역 스코프를 생성하고, 지역 변수, 매개변수, arguments 객체를 관리해야한다. 생성한 지역 스코프를 스코프 체인의 일원으로 연결해야 하기 위해서 함수 실행 컨텍스트가 생성된다.

#### 3. eval 코드

strict mode에서 자신만의 독자적인 스코프를 생성하기 위해 eval 실행 컨텍스트가 생성된다.

#### 4. 모듈 코드

모듈별로 독립적인 모듈 스코프를 생성하기 위해 모듈 실행 컨텍스틀 생성된다.
...
실행컨텍스트를 생성하고 변수, 함수 등의 선언문만 먼저 실행하여 생성된 변수나 함수 식별자를 키로 실행 컨텍스트가 관리하는 스코프인 환경 레코드에 등록한다.

#### 2. 소스코드 실행

선언문을 제외한 소스코드를 실행하는 런타임이 시작된다. 소스코드 실행에 필요한 정보를 실행 컨텍스트가 관리하는 스코프에서 검색하여 취득한다. 그 후 코드 실행 결과로 인한 값의 변경 등을 다시 실행 컨텍스트가 관리하는 스코프에 등록한다.

 

## 실행 컨텍스트 역할

코드가 실행되려면 스코프, 식별자, 코드 실행 순서 등의 관리가 필요하고 그 역할을 실행 컨텍스트가 담당한다. 

실행 컨텍스트는 식별자(변수, 함수, 클래스 등의 이름)를 등록하고 관리하는 스코프와 코드 실행 순서 관리를 구현한 매커니즘이다. 

모든 코드는 이 실행 컨텍스트를 통해 실행되고 관리된다.
...
1. 선언에 의해 생성된 모든 식별자(변수, 함수, 클래스 등)를 스코프를 구분하여 등록하고 상태 변화(식별자 바인딩된 값의 변화)를 지속적으로 관리할 수 있어야 한다.
2. 스코프는 중첩 관계에 의해 스코프 체인을 형성해야 한다. 즉, 스코프 체인을 통해 상위 스코프로 이동하며 식별자를 검색할 수 있어야 한다.
3. 현재 실행 중인 코드의 실행 순서를 변경할 수도, 다시 되돌아 갈 수도 있어야 한다.
...
소스코드가 평가되면 실행 컨텍스트가 생성되고 실행 컨텍스트 스택의 최상위에 쌓인다. **실행 컨텍스트 스택의 최상위에 존재하는 실행 컨텍스트는 언제나 현재 실행 중인 코드의 실행 컨텍스트다.** 따라서 실행 컨텍스트 스택의 최상위에 존재하는 실행 컨텍스트를 **실행 중인 실행 컨텍스트**라 부른다.